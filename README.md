...
* [`src/medal_widget/`](src/medal_widget/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/medal_widget/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `medal-widget` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/medal_widget/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns medal-widget.example
  (:require [medal-widget.config :as config])
